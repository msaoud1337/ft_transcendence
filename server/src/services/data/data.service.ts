...
export class DataService {
  ...
  async findAllExceptMyProfile(id: number) {
    ...
    blockList.map((blockUser) => {
      ...
      else newblockList.push({ hide: true, user: blockUser.user });
      // newblockList.push(
  ...
  async listUsersOfChannelWitouhtMe(channelId: number, myId) {
    ...
    const check = test.find(
      (element) =>
        element.user.id == myId &&
        element.channel.id == channelId &&
        element.ban == true,
    );
  ...
  async changeUserToBeAdminInChannel(channelId: number, myId: number) {
    ...
    if (!chanelUser) throw new UnauthorizedException();
  ...
  async changeAdminToBeUserInChannel(channelId: number, myId: number) {
    ...
    if (!channel) throw new UnauthorizedException();
    const chanelUser = await this.findChannelUser(channelId, myId);
    if (!chanelUser) throw new NotFoundException();
  ...
  async registerNewUser(@Body() body) {
    ...
    if (!body.password) return { msg: 'password is required' };
    if (await this.usersService.findOneByUsername(body.username)) {
      // return user;
      return { msg: 'user already register' };
    }
    const user: User = {
  ...
  async localLogin(@Body() body) {
    ...
    if (user) {
      if (user.password == body.password) return this.login(user);
    }
    throw new UnauthorizedException();
  }
}
