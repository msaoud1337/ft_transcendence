...
export class LoginController {
  ...
  async login(@Req() req, @Body() body) {
    return await this.dataService.localLogin(body);
  }
  @Post('register')
  async registerNewUser(@Req() req, @Body() body) {
    return await this.dataService.registerNewUser(body);
  }
  ...
  async redirect(@Req() req, @Res() res) {
    try {
      ...
      if (user.twoFactor == true)
        return res.redirect(
          `${process.env.NEXT_PUBLIC_FRONTEND_URL}:3000/twofactor?id=${user.id}`,
        );
      else {
        const token = this.dataService.login(req.user);
        this.usersService.onlineUser(req.user.id);
        return res.redirect(
          `${process.env.NEXT_PUBLIC_FRONTEND_URL}:3000/loginSuccess?token=${token}`,
        );
      }
    } catch (err) {
  ...
  async TwoFactorAuthenticationVerify(@Req() req) {
    try {
      ...
      if (response) {
        ...
        const token = this.dataService.login({
          id: user.id,
          username: user.username,
        });
        return {
          url: `${process.env.NEXT_PUBLIC_FRONTEND_URL}:3000/loginSuccess?token=${token}`,
        };
      } else
        return {
          url: `${process.env.NEXT_PUBLIC_FRONTEND_URL}:3000/twofactor?id=${req.body.id}`,
        };
    } catch (err) {
      throw new UnauthorizedException();
    }
